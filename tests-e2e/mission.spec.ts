...
async function clickWorld(page: Page, x: number, y: number) { await page.mouse.click((await worldPoint(page, x, y)).x, (await worldPoint(page, x, y)).y); }
...
async function startFresh(page: Page) {
  ...
  await page.locator('#game-container canvas').waitFor({ state: 'visible' }); await expect(page.getByTestId('game-hud')).toContainText('OFFICE');
}
...
test('completes the adventure with board links, sorting arcade and safe finale', async ({ page }) => {
  const pageErrors: string[] = []; page.on('pageerror', error => pageErrors.push(error.message)); await startFresh(page);
  await clickWorld(page, 330, 390); await page.waitForTimeout(120); await clickWorld(page, 330, 390); await expect(page.getByTestId('world-toast')).toContainText('café'); await clickWorld(page, 390, 455); await page.waitForTimeout(120); await clickWorld(page, 390, 455); await expect(page.getByTestId('world-toast')).toContainText('Llave encontrada'); await clickWorld(page, 885, 390); await expect(page.getByTestId('game-hud')).toContainText('ARCHIVE');
  ...
  for (const x of [180, 405, 630, 855, 1080]) await clickWorld(page, x, 625); await expect(page.getByTestId('conversation-prompt')).toBeVisible(); expect(pageErrors).toEqual([]);
});
